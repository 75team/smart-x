...
void NativeAdapter::postMessage(const char* message, const char* data = "null"){
    ...
    if(std::string(_method->valueString) == "open"){
        Json* _params = Json_getItem(root, "params");
        OpenWeb::open(Json_getString(_params, "url", ""));
    }
    
    else if(std::string(_method->valueString) == "getDeviceInfo"){
    ...
    else if(std::string(_method->valueString) == "getAppInfo"){
        int idValue = Json_getInt(root, "id", 0);
